...
def addDelivSeg(seg_details,mode,GRAPHS,WORLD,mass=1,track=False):
    ...
    start2 = WORLD['main']['end_time']; #seg_details[3]
    end1 = WORLD['main']['start_time']; #seg_details[4]
    end2 = WORLD['main']['end_time']; # end2 = seg_details[5]
    ...
    if not(trip in WORLD[mode]['trips'].keys()):
        ...
        WORLD[mode]['trips'][trip]['am'] = 1;
        WORLD[mode]['trips'][trip]['wc'] = 0;
        WORLD[mode]['trips'][trip]['pickup_time_window_start'] = start1;
        WORLD[mode]['trips'][trip]['pickup_time_window_end'] = start2;
        WORLD[mode]['trips'][trip]['dropoff_time_window_start'] = end1;
        WORLD[mode]['trips'][trip]['dropoff_time_window_end'] = end2;
        booking_id = str(len(WORLD[mode]['booking_ids']));
        ...
        time_matrix = WORLD[mode]['time_matrix']; shp = np.shape(time_matrix)
        time_matrix = np.block([[time_matrix,np.zeros([shp[0],2])],[np.zeros([2,shp[1]]),np.zeros([2,2]) ]]);
        WORLD[mode]['time_matrix'] = time_matrix;
...
def planGTFSSeg(trip0,mode,GRAPHS,WORLD,NODES,mass=1,track=False,verbose=False):
...
def querySeg(seg_details,mode,PERSON,NODES,GRAPHS,WORLD):
    """
    ...
    if not((start,end) in WORLD[mode]['trips']):#.keys()):  
        if mode == 'gtfs':
            ...
            addDelivSeg((start,end),mode,GRAPHS,WORLD,mass=0);
...
#         start_type = start_types[k];
        for j,end in enumerate(end_nodes):
            ...
            for i,start in enumerate(start_nodes):                
                ...
                cost,_ = querySeg(seg_details,mode,PERSON,NODES,GRAPHS,WORLD)
...
def travel_time_matrix(nodes,GRAPH):
    MAT = np.zeros([len(nodes),len(nodes)]);
    for i,node1 in enumerate(nodes):
        distances, paths = multi_source_dijkstra(GRAPH, nodes, target=node1, weight='c');
...
def constructPayload(active_trips,DELIVERY,WORLD,GRAPHS): #PAY,ids_to_keep):
    payload = {};
    ...
    for i,trip in enumerate(active_trips):
        ...
        requests[i]['pickup_node_id'] = TRIP['pickup_node_id']
        requests[i]['dropoff_node_id'] = TRIP['dropoff_node_id']

        requests[i]['am'] = TRIP['am']
        requests[i]['wc'] = TRIP['wc']
        ...
        nodes[node1] = TRIP['pickup_node_id']
        nodes[node2] = TRIP['dropoff_node_id']
...
def payloadDF(PAY,GRAPHS,include_drive_nodes = False):
...
def assignCostsFromManifest(active_trips,nodes,MDF,WORLD,track=True):
    mode = 'ondemand';
    for i,trip in enumerate(active_trips):
        node1 = trip[0]
        node2 = trip[1]
        node_id1 = nodes[trip[0]]
        node_id2 = nodes[trip[1]]
        mask1 = (MDF['node_id'] == node_id1) & (MDF['action'] == 'pickup');
        mask2 = (MDF['node_id'] == node_id2) & (MDF['action'] == 'dropoff');
        time1 = MDF[mask1]['scheduled_time'][0];
        time2 = MDF[mask2]['scheduled_time'][0];

        WORLD[mode]['trips'][trip]['costs']['current_time'] = time2-time1;

        # print(WORLD[mode]['trips'][trip]['costs'])#['current_time'] = time2-time1;
...
def generateDeliveries(GRAPHS,NDF,params):
    ...
    num_drivers = 8; 
    driver_start_time = 43500;
    driver_end_time = 73500;
    am_capacity = 8;
    wc_capacity = 2;
...
def world_of_ondemand(WORLD,PEOPLE,DELIVERY,GRAPHS,verbose=False,show_delivs='all'):
    ...
    if True:
        ...
        trips_to_plan = WORLD['ondemand']['active_trips'];#_direct'] + WORLD['ondemand']['active_trips_shuttle'];
        ...
        if len(trips_to_plan)>0:
            ...
            #### manifest = optimizePayload(payload) ####
            manifest = fakeManifest(payload)

            PDF = payloadDF(payload,GRAPHS)
            MDF = manifestDF(manifest,PDF)
            assignCostsFromManifest(trips_to_plan,nodes,MDF,WORLD)
...
# def plotPAYLOAD(GRAPHS,NODES,EDGES,scale=1.):
#     bgcolor = [0.8,0.8,0.8,1];
#     node_colors = [];
#     node_sizes = [];
...
#         node_colors.append([1,1,1,1]);
#         node_sizes.append(0);
#         if node in NODES['pickup']:        
#             node_colors[-1] = [0,0,1,0.5]
#             node_sizes[-1] = scale*1; #.append(scale*home_sizes[node]);            
#         elif node in NODES['dropoff']:
#             node_colors[-1] = [1,0,0,0.5]
#             node_sizes[-1] = scale*1; #.append(scale*home_sizes[node]);            
